...
Exercises GET /status, POST /mode, and POST /speed via a TestClient with
mocked controller and config dependencies.  No real hardware or I2C.
"""
...
def _make_client():
    """Create a TestClient with mocked fan route dependencies."""
    ...
    # _get_config is a callable that returns the config manager
    get_config = MagicMock(return_value=config_manager)

    routes.configure(controller=controller, get_config=get_config)

    app = FastAPI()
    app.include_router(routes.router)
    client = TestClient(app)
    return client, controller, config_manager, get_config
...
class TestFanStatus:
    ...
    def test_fan_status_503_when_not_configured(self):
        """If configure() was never called, controller is None -> 503."""
        routes._controller = None
        app = FastAPI()
        app.include_router(routes.router)
...
class TestSetFanMode:
    ...
    def test_set_fan_mode_503_when_not_configured(self):
        ...
        # Controller still needs to be set for the app to work,
        # but _get_config is the guard for POST /mode.
        app = FastAPI()
        app.include_router(routes.router)
        client = TestClient(app, raise_server_exceptions=False)
        resp = client.post("/mode", json={"mode": 0})
        assert resp.status_code == 503
...
class TestSetFanSpeed:
    """Tests for the POST /speed endpoint."""

    def test_set_fan_speed_valid(self):
        client, _, config_manager, get_config = _make_client()
        resp = client.post("/speed", json={"duty": [50, 60, 70]})
        assert resp.status_code == 200
        data = resp.json()
        assert data["status"] == "ok"
    ...
    def test_set_fan_speed_pads_to_three_channels(self):
        ...
        client, _, config_manager, _ = _make_client()
        resp = client.post("/speed", json={"duty": [100]})
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_set_fan_speed_503_when_not_configured(self):
        ...
        app.include_router(routes.router)
        client = TestClient(app, raise_server_exceptions=False)
        resp = client.post("/speed", json={"duty": [50]})
        assert resp.status_code == 503
